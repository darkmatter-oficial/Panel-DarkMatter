if _G.keyValida and _G.textoAdvertenciaFirebase.Body == '""' then
...
local streetLifeId = "71600459831333"

-- arsenal no tiene la carpeta controls --
...
if gameId == killStreakId or gameId == streetLifeId then
  player:Kick("Experiencia PELIGROSA detectada, lo mejor es NO usar el panel DarkMatter en esta experiencia para evitar BANEOS o arruinar la experiencia")
end
...
local opciones = {
  ...
  grandeDeLaHitBox = 50,
  expandHitboxEnable = false,
  velocitySpeed = 50,
  jumpPower = 100,
  aimExpDuelistEnable = false,
  autoShootEnable = false,
  silentAimActived = false,
  aimbotBodyEnable = false,
  modosSilent = {
    "100%",
    "70%",
    "50%",
...
runService.RenderStepped:Connect(function()
  ...
  if opciones.espTracerEnable then 
  ...
  if opciones.espVidaEnable then
  ...
  if opciones.showFovEnable then
    if fov then
  ...
  if opciones.expandHitboxEnable then
    for _, jugador in pairs(players:GetPlayers()) do
      if jugador.Character then
  ...
  if opciones.aimExpDuelistEnable then
    apuntarATarget()
  end
  
  if opciones.autoShootEnable then
  ...
  if opciones.aimbotBodyEnable then
    for _, jugador in pairs(players:GetPlayers()) do
      if jugador.Character then
        local jugadorCharacter = jugador.Character
...
-- Actualizar las Hitbox de el Expand Hitbox --
...
-- Actualizar la posición a tiempo real con el boton del logo --

  menuBackground.Position = UDim2.new(0, minimizarButton.AbsolutePosition.X + 25, 0, minimizarButton.AbsolutePosition.Y + 30)
  
-- Detectar cuando el panel entra en mantenimiento --

  if _G.textoAdvertenciaFirebase.Body ~= '""' then
    menuBackground.Visible = false
    minimizarButton.Visible = false
    task.wait(1)
    player:Kick("EL PANEL DARKMATTER ENTRO EN MANTENIMIENTO")
  end
...
if gameId ~= rivalsId then
  crearSwitch(menuSection2, "Aimbot", "COMBATE", function(valor)
    if valor then
      opciones.aimbotEnable = true
    else
      opciones.aimbotEnable = false
    end
  end)
...
crearSwitch(menuSection2, "Esp Tracer", "VISUALES", function(valor)
  if valor then
    ...
    opciones.espTracerEnable = false
...
crearSwitch(menuSection2, "Esp Vida", "VISUALES", function(valor)
  if valor then
    ...
    opciones.espVidaEnable = true
  else
    opciones.espVidaEnable = false
...
crearSwitch(menuSection2, "Show Fov", "VISUALES", function(valor)
  if valor then
    opciones.showFovEnable = true
  else
    opciones.showFovEnable = false
...
if gameId ~= rivalsId then
  crearSwitch(menuSection2, "Super Jump", "MOVIMIENTO", function(valor)
    if valor then
      opciones.superJumpActivado = true
    else
      opciones.superJumpActivado = false
      if player.Character then
        if player.Character.Humanoid then
...
if gameId ~= rivalsId then
  crearSwitch(menuSection2, "Super Speed", "MOVIMIENTO", function(valor)
    ...
    if valor then
      opciones.superSpeedActivado = true
    else
      opciones.superSpeedActivado = false
      if player.Character then
        if player.Character.Humanoid then
...
crearSwitch(menuSection2, "NoClip", "MOVIMIENTO", function(valor)
  if valor then
    opciones.noClipEnable = true
  else
    opciones.noClipEnable = false
    if player.Character then
      local character = player.Character
      for _, v in pairs(character:GetChildren()) do
        if v:IsA("BasePart") then
...
if plataforma.Name == "Android" or plataforma.Name == "IOS" then
  if gameId ~= arsenalId then
    crearSwitch(menuSection2, "Fly", "MOVIMIENTO", function(valor)
      if valor then
        opciones.flyEnable = true
      else
        opciones.flyEnable = false
        if player.Character then
          if player.Character.Humanoid then
...
crearSwitch(menuSection2, "gravity 0", "MOVIMIENTO", function(valor)
  if valor then
    opciones.GravityCeroActivado = true
  else
    opciones.GravityCeroActivado = false
...
if gameId ~= rivalsId then
  crearSwitch(menuSection2, "Expand Hitbox", "ENEMY CONFIG", function(valor)
    if valor then
      opciones.expandHitboxEnable = true
    else
      opciones.expandHitboxEnable = false
      for _, jugador in pairs(players:GetPlayers()) do
        restaurarHitBox(jugador.Character)
      end
    end
...
if gameId == duelistId then
  crearSwitch(menuSection2, "Aim Exp (Duelist)", "COMBATE", function(valor)
    if valor then
      opciones.aimExpDuelistEnable = true
    else
      opciones.aimExpDuelistEnable = false
    end
  end)
end
...
if gameId == duelistId and (plataforma == Enum.Platform.IOS or plataforma == Enum.Platform.Android) then
  crearSwitch(menuSection2, "Auto Shoot", "ENEMY CONFIG", function(valor)
    if valor then
      opciones.autoShootEnable = true
    else
      opciones.autoShootEnable = false
    end
  end)
end
...
if gameId ~= rivalsId then
  crearSwitch(menuSection2, "Aimbot Body", "ENEMY CONFIG", function(valor)
    if valor then
      opciones.aimbotBodyEnable = true
    else
      opciones.aimbotBodyEnable = false
      for _, jugador in pairs(players:GetPlayers()) do
...
-- OPTIMIZAR EL PANEL QUITANDO TODOS LOS EFECTOS Y TEXTURAS --

local lighting = game:GetService("Lighting")

local function optimizar(objeto)
  if objeto:IsA("Texture") or objeto:IsA("Decal") or objeto:IsA("BasePart") or objeto:IsA("ParticleEmitter") or objeto:IsA("Smoke") or objeto:IsA("Fire") or objeto:IsA("Sparkles") or objeto:IsA("PointLight") or objeto:IsA("SpotLight") or objeto:IsA("SurfaceLight") or objeto:IsA("Beam") or objeto:IsA("Trail") then
    if not objeto:IsA("BasePart") and not objeto:IsA("ParticleEmitter") then
      objeto:Destroy()
    end
    ...
    if objeto:IsA("Smoke") or objeto:IsA("Fire") or objeto:IsA("Sparkles") or objeto:IsA("PointLight") or objeto:IsA("SpotLight") or objeto:IsA("SurfaceLight") or objeto:IsA("Beam") or objeto:IsA("Trail") then
...
for _, efecto in ipairs(lighting:GetChildren()) do
  if efecto:IsA("BloomEffect") or efecto:IsA("BlurEffect") or efecto:IsA("ColorCorrectionEffect") or efecto:IsA("SunRaysEfect") or efecto:IsA("DepthOfFieldEffect") or efecto:IsA("Atmosphere") then
    efecto:Destroy()
  end
end
