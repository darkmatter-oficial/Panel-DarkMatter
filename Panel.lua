...
-- Crear el icono del panel en la esquina --

local icon = Instance.new("ImageLabel")
icon.Parent = menuBackground
...
-- Contenedor de el slider y el nombre de la opcion --
  ...
  sliderAndTextContainer.BackgroundTransparency = 1
...
minimizarButton.Parent = menuBackground

minimizarButton.BackgroundColor3 = Color3.fromRGB(28, 18, 48)
minimizarButton.Size = UDim2.new(0, 25, 0, 25)
minimizarButton.Position = UDim2.new(0, 415, 0, 5)
minimizarButton.Text = "+"
minimizarButton.TextColor3 = Color3.fromRGB(71, 32, 112)
minimizarButton.TextSize = 25
minimizarButton.Font = Enum.Font.ArialBold
minimizarButton.ZIndex = zIndex
redondear(minimizarButton, 0, 5)
borde(minimizarButton, 2, 31, 18, 51)
...
minimizarButton.MouseButton1Click:Connect(function()
  ...
  if menuDesplegado then
    ...
    local menuOn = tweenService:Create(
      ...
      TweenInfo.new(0.5),
      {
        Position = UDim2.new(actualPosition.X.Scale, actualPosition.X.Offset, actualPosition.Y.Scale, actualPosition.Y.Offset - 133),
        ...
        if character:FindFirstChild("HumanoidRootPart") then
          ...
          if opciones.teamCheck then
            if opciones.wallCheckActivado then
              if distancia < distanciaMasCercana and parteVisible(jugadorHead) and vida > 0 and jugadorEnElFov and diferentesEquipos(jugador)then
            ...
            if opciones.wallCheckActivado then
              if distancia < distanciaMasCercana and parteVisible(jugadorHead) and vida > 0 and jugadorEnElFov then
                distanciaMasCercana = distancia
                jugadorMasCercano = jugadorHead
              end
            else
  ...
  for _, jugador in pairs(players:GetPlayers()) do
    if jugador.Character and jugador ~= player then
      ...
      if jugadorCharacter:FindFirstChild("Head") then
        ...
        if character:FindFirstChild("HumanoidRootPart") then
          local distancia = (character.HumanoidRootPart.Position - jugadorHead.Position).Magnitude
          if distancia < distanciaMasCercana and parteVisible(jugadorHead) and vida > 0 then
            distanciaMasCercana = distancia
            jugadorMasCercano = jugadorHead
          end
        end
...
local function expandirHitBox(character)
  if character ~= player.Character and character then
    ...
    if jugadorChar:FindFirstChild("HumanoidRootPart") then
      ...
      jugadorHrp.Size = Vector3.new(opciones.grandeDeLaHitBox, opciones.grandeDeLaHitBox, opciones.grandeDeLaHitBox)
      jugadorHrp.Transparency = 0.5
      jugadorHrp.Color = Color3.fromRGB(255, 0, 0)
      jugadorHrp.Material = Enum.Material.Neon
      jugadorHrp.CanCollide = false
    end
  end
end
...
local function apuntarATarget()
  ...
  if targetShoot then
    ...
    if distanciaAlTarget < 2000 and visible then 
...
old = hookmetamethod(game, "__index", newcclosure(function(self, key)
  if self == camera and key == "ViewportSize" and opciones.silentAimActived and posSilent and enPantallaSilent then
...
if gameId ~= rivalsId and gameId ~= duelistId then
...
1. Agregar el Team Check para duelist

2.  Agregar cambio de modo de aimbot (pecho, cuello, cabeza)
